...
crate_dir=$(cd "$bootstrap_dir/.." && pwd)
# Resolved dynamically from this crate's bootstrap directory.
# shellcheck disable=SC1091
source "$bootstrap_dir/lock.sh"
build_root=${LECTOR_GHOSTTY_BUILD_ROOT:-"$crate_dir/target"}
target=$(rustc -vV | sed -n 's/^host: //p')
optimize=Debug
prebuilt_root=${GHOSTTY_PREBUILT_ROOT:-"$build_root/ghostty-prebuilt"}
...
if [[ "$actual_zig" != "$REQUIRED_ZIG_VERSION" ]]; then
    zig_bin=$("$bootstrap_dir/bootstrap_zig.sh")
fi
