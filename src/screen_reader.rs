    ...
    fn autoread(
        ...
        let diffing = match text {
            ...
            _ => {
                ...
                if echoed_char {
                    false
                } else if screen_state.screen.contents() == screen_state.prev_screen.contents()
                    && screen_state.screen.cursor_position()
                        != screen_state.prev_screen.cursor_position()
                {
                    // If the screen's contents haven't changed, the text might just be parts of
                    // the screen re-drawn to change attributes.
                    // But if the cursor also hasn't moved, it could be that the whole screen has
                    // scrolled, and the next screen has the same text.
                    false
                } else {
                    self.speech.speak("asdf: text", false)?;
        ...
        if diffing {
            ...
            self.speech.speak("asdf: diff", false)?;
            self.speech.speak(&text, false)?;
            text_read = Some(&text);
        }
